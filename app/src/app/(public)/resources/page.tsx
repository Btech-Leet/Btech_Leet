...
import ResourcePurchaseButton from "@/components/resources/purchase-button";
...
export default async function ResourcesPage() {
  ...
  let purchasedResourceIds = new Set<string>();
  if (user) {
    try {
      ...
      purchasedResourceIds = new Set(transactions.map(t => t.purchaseItemId));
    } catch (err) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {resources.length === 0 ? (
            ...
            resources.map((res: any) => {
              const isPaid = res.price && res.price > 0;
              const hasAccess = !isPaid || (user && (user.role === "ADMIN" || user.role === "EDITOR" || purchasedResourceIds.has(res.id)));
