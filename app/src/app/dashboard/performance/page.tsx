    ...
    <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 border-b border-gray-900 pb-6">
            <div>
              ...
              <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-white via-gray-100 to-gray-400">
                Your LEET Exam Insights
              </h1>
              <p className="text-gray-400 text-sm mt-1">Detailed evaluation of test performance, study habits, predictions, and state ranking.</p>
            </div>
            
            <div className="flex items-center gap-3">
              <Button 
                onClick={() => setShowLogModal(true)}
                className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 shadow-md transition-all font-semibold rounded-xl"
              >
                Log Study Session
              </Button>
              <Button 
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              { label: "Overall Accuracy", value: `${overview.overallAccuracy}%`, desc: "Correct answers", icon: Target, color: "text-blue-400", bg: "bg-blue-950/20 border-blue-900/30" },
              { label: "Tests Attempted", value: overview.totalTests, desc: "Mock tests taken", icon: BookOpen, color: "text-purple-400", bg: "bg-purple-950/20 border-purple-900/30" },
              { label: "Consecutive Study Streak", value: `${streaks.currentStreak} Days`, desc: "Study streak", icon: Flame, color: "text-amber-500", bg: "bg-amber-950/20 border-amber-900/30" },
              { label: "Global Ranking", value: `#${overview.currentRank}`, desc: "Overall leaderboard rank", icon: Trophy, color: "text-emerald-400", bg: "bg-emerald-950/20 border-emerald-900/30" }
            ].map((stat) => {
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            ...
            {/* Tabs List Sidebar */}
            <div className="w-full lg:w-64 flex flex-row lg:flex-col overflow-x-auto lg:overflow-x-visible gap-2 border-b lg:border-b-0 lg:border-r border-gray-900 pb-4 lg:pb-0 lg:pr-6 flex-shrink-0 scrollbar-none">
              {[
            ...
            <div className="flex-1 w-full min-w-0">
                ...
                <div className="space-y-8 animate-fadeIn">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <Card className="bg-gray-900 border-gray-800 rounded-2xl relative overflow-hidden">
                      ...
                      <CardHeader className="flex flex-row justify-between items-center pb-2">
                        <div>
                          <CardTitle className="text-white text-base">Targets & Goals</CardTitle>
                          <CardDescription className="text-gray-400 text-xs">Track progress against exam targets</CardDescription>
                        </div>
                        <Button 
                          variant="ghost" 
                          size="sm" 
                          ...
                          <form onSubmit={saveGoals} className="space-y-4">
                            <div className="grid grid-cols-3 gap-3">
                              <div className="space-y-1">
                                <label className="text-[10px] text-gray-400 font-bold uppercase">Target Rank</label>
                                <input type="number" value={targetRank} onChange={(e) => setTargetRank(parseInt(e.target.value))} className="w-full bg-gray-950 border border-gray-800 rounded-lg p-2 text-sm text-white" />
                              </div>
                              <div className="space-y-1">
                                <label className="text-[10px] text-gray-400 font-bold uppercase">Target %</label>
                                <input type="number" value={targetScore} onChange={(e) => setTargetScore(parseInt(e.target.value))} className="w-full bg-gray-950 border border-gray-800 rounded-lg p-2 text-sm text-white" />
                              </div>
                              <div className="space-y-1">
                                <label className="text-[10px] text-gray-400 font-bold uppercase">Daily Goal (Mins)</label>
                                <input type="number" value={dailyGoal} onChange={(e) => setDailyGoal(parseInt(e.target.value))} className="w-full bg-gray-950 border border-gray-800 rounded-lg p-2 text-sm text-white" />
                              </div>
                            </div>
                            <Button type="submit" size="sm" className="bg-blue-600 hover:bg-blue-700 w-full rounded-lg text-xs font-semibold">Save Goals</Button>
                          </form>
                        ) : (
                          <div className="space-y-4">
                            <div className="space-y-2">
                              <div className="flex justify-between text-xs font-semibold">
                                <span className="text-gray-400 flex items-center gap-1"><Clock size={12} /> Daily Study Habit</span>
                                <span>{Math.min(100, Math.round((overview.dailyStudyTime / dailyGoal) * 100))}% ({overview.dailyStudyTime}/{dailyGoal} mins)</span>
                              </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-2 border-t border-gray-800">
                              <div>
                                <span className="text-[10px] text-gray-400 font-semibold uppercase block">Target Rank</span>
                                <span className="text-lg font-extrabold text-white"># {targetRank}</span>
                                <span className="text-[9px] text-gray-500 block">Current estimated: #{predictions.expectedRank}</span>
                              </div>
                              <div>
                                <span className="text-[10px] text-gray-400 font-semibold uppercase block">Target Score</span>
                                <span className="text-lg font-extrabold text-white">{targetScore}%</span>
                                <span className="text-[9px] text-gray-500 block">Current average: {predictions.expectedScore}%</span>
                              </div>
                            </div>
                          </div>
                        )}
                    ...
                    <Card className="bg-gray-900 border-gray-800 rounded-2xl relative overflow-hidden">
                      ...
                      <CardHeader className="pb-2">
                        <CardTitle className="text-white text-base">Study Streaks</CardTitle>
                        <CardDescription className="text-gray-400 text-xs">Consistent habits build success</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        <div className="flex justify-between items-center p-3.5 bg-gray-950 rounded-xl border border-gray-850">
                          <div className="flex items-center gap-3">
                            <div className="p-2 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-lg">
                              <Flame size={18} className="animate-pulse" />
                            </div>
                            <div>
                              <p className="text-xs font-semibold text-white">Study Streak</p>
                              <p className="text-[10px] text-gray-500">Consecutive study logs</p>
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div className="p-3 bg-gray-950 border border-gray-850 rounded-xl flex flex-col justify-between">
                            <span className="text-[10px] text-gray-400 font-semibold uppercase">Consecutive Logins</span>
                            <span className="text-lg font-extrabold text-white mt-1">{streaks.consecutiveLogins} Days</span>
                          </div>
                          <div className="p-3 bg-gray-950 border border-gray-850 rounded-xl flex flex-col justify-between">
                            <span className="text-[10px] text-gray-400 font-semibold uppercase">Consecutive Tests</span>
                            <span className="text-lg font-extrabold text-white mt-1">{streaks.consecutiveTests} Days</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                  ...
                  <Card className="bg-gray-900 border-gray-800 rounded-2xl">
                    <CardHeader>
                      ...
                      <CardDescription className="text-gray-400 text-xs">Total study hours, questions solved, and progress over the last week</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="h-[280px]">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={dailyStudyReport} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <YAxis stroke="#9ca3af" fontSize={11} label={{ value: "Minutes", angle: -90, position: "insideLeft", fill: "#9ca3af", fontSize: 10 }} />
                  ...
                  {/* Row 3: Weak / Strong Topics */}
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    
                    {/* Strong Topics */}
                    <Card className="bg-gray-900 border-gray-800 rounded-2xl border-l-4 border-l-green-500">
                      <CardHeader className="pb-2">
                        <CardTitle className="text-white text-base flex items-center gap-2">
                          <CheckCircle2 className="text-green-500" size={18} />
                          Strong Topics (High Accuracy)
                        </CardTitle>
                        <CardDescription className="text-gray-400 text-xs">Subjects and topics where accuracy is above 80%</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-3">
                        {strongTopics.map((topic: any, idx: number) => (
                          <div key={`${topic.topic}-${idx}`} className="p-3 bg-gray-950 border border-gray-850 rounded-xl flex items-center justify-between">
                            <div>
                              <p className="text-xs font-bold text-white">{topic.topic}</p>
                              <p className="text-[10px] text-gray-500">{topic.subject}</p>
                            </div>
                    ...
                    {/* Weak Topics */}
                    <Card className="bg-gray-900 border-gray-800 rounded-2xl border-l-4 border-l-red-500">
                      <CardHeader className="pb-2">
                        <CardTitle className="text-white text-base flex items-center gap-2">
                          <ShieldAlert className="text-red-500" size={18} />
                          Weak Topics (Review Needed)
                        </CardTitle>
                        <CardDescription className="text-gray-400 text-xs">Topics below 60% accuracy. Practice papers suggested</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-3">
                        {weakTopics.map((topic: any, idx: number) => (
                          <div key={`${topic.topic}-${idx}`} className="p-3 bg-gray-950 border border-gray-850 rounded-xl space-y-2">
                            ...
                            <div className="flex gap-2 pt-1 border-t border-gray-900/50">
                              <a href="/papers" className="inline-flex items-center text-[10px] text-blue-400 hover:underline gap-0.5 font-semibold">
                                <FileText size={10} /> Practice Papers
                              </a>
                              <a href="/resources" className="inline-flex items-center text-[10px] text-purple-400 hover:underline gap-0.5 font-semibold">
              ...
              {activeTab === "subjects" && (
                <div className="space-y-6 animate-fadeIn">
                  ...
                  <Card className="bg-gray-900 border-gray-800 rounded-2xl">
                    <CardHeader>
                      <CardTitle className="text-white text-base">Subject Accuracy Breakdown</CardTitle>
                      <CardDescription className="text-gray-400 text-xs">Evaluate accuracy percentage across all LEET core subjects</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="h-[280px]">
                        <ResponsiveContainer width="100%" height="100%">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {subjectPerformance.map((sub: any, idx: number) => (
                      <div key={sub.subject} className="p-5 bg-gray-900 border border-gray-800 rounded-2xl space-y-3">
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="text-sm font-bold text-white">{sub.subject}</h3>
                            <p className="text-xs text-gray-500">Attempted {sub.attempts} questions</p>
                          </div>
                          <div className="text-right">
                            <span className="text-lg font-bold block" style={{ color: COLORS[idx % COLORS.length] }}>
                              {sub.accuracy}%
                            </span>
                            <span className="text-[9px] text-gray-400 font-semibold block uppercase">Estimated Rank: #{sub.rank}</span>
                          </div>
                        </div>
                        <p className="text-xs text-gray-400 leading-relaxed p-3 bg-gray-950 rounded-xl border border-gray-850">
                          {sub.suggestions}
                        </p>
                      </div>
                    ))}
                  ...
                  <Card className="bg-gray-900 border-gray-800 rounded-2xl">
                    <CardHeader>
                      ...
                      <CardDescription className="text-gray-400 text-xs">Compare your average score with other engineering branch averages</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="h-[280px]">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={branchPerformance} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <Bar dataKey="averageScore" name="Branch Avg Score" fill="#1f2937" stroke="#374151" strokeWidth={1} radius={[4, 4, 0, 0]} />
              ...
              {activeTab === "predictions" && (
                <div className="space-y-6 animate-fadeIn">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    {/* Readiness */}
                    <Card className="bg-gray-900 border-gray-800 rounded-2xl flex flex-col justify-between">
                      <CardHeader className="pb-2">
                        <CardTitle className="text-white text-base">Exam Readiness</CardTitle>
                        <CardDescription className="text-gray-400 text-xs">Percent ready for final LEET exam</CardDescription>
                      </CardHeader>
                      <CardContent className="flex flex-col items-center justify-center py-6">
                        <div className="relative w-36 h-36 flex items-center justify-center">
                          <svg className="w-full h-full transform -rotate-90">
                            <circle cx="72" cy="72" r="62" stroke="#1f2937" strokeWidth="8" fill="transparent" />
                          ...
                          <div className="absolute text-center">
                            <span className="text-3xl font-black text-white">{overview.examReadiness}%</span>
                            <span className="text-[10px] text-gray-400 block font-semibold uppercase mt-0.5">Ready</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    {/* Rank Predictor */}
                    <Card className="bg-gray-900 border-gray-800 rounded-2xl md:col-span-2 relative overflow-hidden flex flex-col justify-between">
                      <div className="absolute top-0 right-0 p-4 opacity-5">
                        <Trophy size={160} />
                      </div>
                      <CardHeader className="pb-2">
                        <CardTitle className="text-white text-base">State Rank & Score Predictor</CardTitle>
                        <CardDescription className="text-gray-400 text-xs">Estimated based on test performance and state competition trends</CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-4 bg-gray-950 border border-gray-850 rounded-xl">
                            ...
                            <span className="text-2xl font-black text-blue-400 block mt-1">{predictions.expectedScore}%</span>
                            <span className="text-[9px] text-gray-400 block mt-1">Accuracy proxy score</span>
                          </div>
                          
                          <div className="p-4 bg-gray-950 border border-gray-850 rounded-xl">
                            <span className="text-[10px] text-gray-500 font-bold uppercase">Expected State Rank</span>
                            <span className="text-2xl font-black text-amber-500 block mt-1"># {predictions.expectedRank}</span>
                            <span className="text-[9px] text-gray-400 block mt-1">Out of ~15,000 candidates</span>
                          </div>
                        </div>

                        <div className="p-4 bg-gray-950 border border-gray-850 rounded-xl flex items-center justify-between">
                          <div>
                            <span className="text-[10px] text-gray-500 font-bold uppercase">Admission Chances</span>
                  ...
                  <Card className="bg-gray-900 border-gray-800 rounded-2xl">
                    <CardHeader>
                      <CardTitle className="text-white text-base">Detailed Result Analytics</CardTitle>
                      <CardDescription className="text-gray-400 text-xs">Analysis of correct/wrong answer counts and time taken per test</CardDescription>
                    </CardHeader>
                    <CardContent className="p-0 overflow-x-auto">
                      {testWisePerformance.length === 0 ? (
                        <p className="text-gray-500 text-sm text-center py-10">Attempt mock tests to populate detailed result breakdown.</p>
                      ) : (
                        <table className="w-full text-left border-collapse text-xs">
                          ...
                          <tbody>
                            {testWisePerformance.map((row: any) => (
                              <tr key={row.id} className="border-b border-gray-850 hover:bg-gray-850/20">
                                <td className="p-4 text-white font-medium">{row.testName}</td>
                                <td className="p-4 text-center text-blue-400 font-bold">{row.score} / {row.totalMarks}</td>
                                <td className="p-4 text-center text-emerald-400 font-semibold">{row.accuracy}%</td>
                      ...
                      <table className="w-full text-left border-collapse text-xs">
                        ...
                        <tbody>
                          {leaderboards[leaderboardTab].map((row: any) => (
                            <tr 
                              key={row.name} 
                              className={`border-b border-gray-850 hover:bg-gray-850/20 ${
                            ...
                            >
                              ...
                              {leaderboardTab === "overall" && (
                                <td className="p-4 text-gray-400">
                                  <div className="truncate max-w-[180px]">{row.college}</div>
                                  <div className="text-[10px] text-gray-500">{row.state}</div>
                                </td>
                              )}
                              <td className="p-4 text-center font-bold text-blue-400">{row.score}%</td>
                              <td className="p-4 text-center text-emerald-400 font-semibold">{row.accuracy}%</td>
                            </tr>
                          ))}
                        </tbody>
                ...
                <div className="space-y-4 animate-fadeIn">
                  <Card className="bg-gray-900 border-gray-800 rounded-2xl">
                    <CardHeader className="pb-2 flex flex-row justify-between items-center">
                      <div>
                        <CardTitle className="text-white text-base">Test History Archive</CardTitle>
                        <CardDescription className="text-gray-400 text-xs">History of all attempted mock exams</CardDescription>
                      </div>
                    </CardHeader>
                    <CardContent className="p-0 overflow-x-auto">
                      {testWisePerformance.length === 0 ? (
                        <div className="text-center py-12">
                          <p className="text-gray-500 text-sm">You haven&apos;t taken any mock tests yet.</p>
                          <a href="/mock-tests">
      ...
      {showLogModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[150] flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl w-full max-w-md overflow-hidden animate-slide-up">
            <div className="p-6 border-b border-gray-800">
              <h3 className="text-lg font-bold text-white">Log Today&apos;s Study Progress</h3>
              <p className="text-xs text-gray-400 mt-1">Track your study time and daily solved questions.</p>
            </div>
            
            <form onSubmit={handleLogStudy} className="p-6 space-y-4">
              <div className="space-y-1.5">
                <label htmlFor="logTime" className="text-xs text-gray-400 font-bold uppercase">Study Time (Minutes)</label>
                <input 
                  type="number" 
                  id="logTime" 
                  value={logTime} 
              ...
              <div className="space-y-1.5">
                <label htmlFor="logQuestions" className="text-xs text-gray-400 font-bold uppercase">Questions Solved</label>
                <input 
                  type="number" 
                  id="logQuestions" 
                  value={logQuestions} 
              ...
              <div className="space-y-1.5">
                <label htmlFor="logNotes" className="text-xs text-gray-400 font-bold uppercase">Study Notes / Chapters Viewed</label>
                <input 
                  type="number" 
                  id="logNotes" 
                  value={logNotes} 
              ...
              <div className="flex gap-3 pt-4 border-t border-gray-800 mt-6">
                <Button 
                  ...
                  className="flex-1 border-gray-800 text-gray-300 hover:text-white"
                >
                  Cancel
                </Button>
                <Button 
