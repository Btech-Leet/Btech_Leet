    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Add faculty, mentors, and subject experts to showcase on the website</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 bg-gray-900 border border-gray-800 rounded-xl p-6 h-fit shadow-sm space-y-5">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="flex flex-col items-center p-4 bg-gray-950 border border-gray-800 rounded-xl gap-3">
              <div className="relative w-24 h-24 rounded-full bg-gray-900 border border-gray-800 flex items-center justify-center text-gray-500 text-3xl font-bold overflow-hidden shadow-inner">
                {imageUploading ? (
            ...
            <div className="grid grid-cols-2 gap-4">
              {/* Order */}
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-gray-400 uppercase">Display Order</label>
                <input
                  type="number"
                  value={order}
            ...
            {/* Experience */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-gray-400 uppercase">Experience / Specialization</label>
              <textarea
                value={experience}
            ...
            {/* Description/Bio */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-bold text-gray-400 uppercase">Short Bio / Description</label>
              <textarea
                value={description}
        ...
        <div className="lg:col-span-2 space-y-4">
          {experts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {experts.map((expert) => (
                <div key={expert.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden flex flex-col justify-between shadow-sm">
                  
                  {/* Expert Card Header */}
                  <div className="p-5 flex gap-4 items-start">
                    <div className="w-16 h-16 rounded-full bg-gray-950 border border-gray-800 overflow-hidden flex-shrink-0 flex items-center justify-center">
                      {expert.photo ? (
                    ...
                    <div className="min-w-0">
                      <h3 className="font-bold text-gray-200 text-base truncate">{expert.name}</h3>
                      {expert.designation && (
                        <p className="text-xs text-violet-400 font-semibold mt-0.5 truncate">{expert.designation}</p>
                      )}
                      {expert.qualification && (
                        <p className="text-xs text-gray-400 mt-0.5 truncate">{expert.qualification}</p>
                      )}
                      {expert.linkedinUrl && (
                        <a
                  ...
                  {expert.description && (
                    <div className="px-5 pb-4">
                      <p className="text-xs text-gray-400 line-clamp-2">{expert.description}</p>
                    </div>
                  )}
