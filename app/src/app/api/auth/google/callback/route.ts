...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!user) {
      ...
      // Verify email retroactively for existing users signing in via Google
      await prisma.user.update({ where: { id: user.id }, data: { emailVerified: true, avatar: picture || user.avatar } });
    }
